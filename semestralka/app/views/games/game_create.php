    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="mb-12">
            <h2 class="text-4xl font-black text-white tracking-tight uppercase italic">
                Přidat <span class="text-indigo-500">Novou Hru</span>
            </h2>
            <p class="text-slate-400 mt-2 font-medium">Vyplňte detaily hry, kterou chcete přidat do katalogu.</p>
        </div>

        <form action="<?= BASE_URL ?>/index.php?url=game/store" method="POST" enctype="multipart/form-data" class="bg-slate-900 rounded-3xl p-8 lg:p-12 border border-slate-800 shadow-2xl">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="title" name="title" required placeholder="Gears of War, Halo, atd."
                           class="w-full bg-slate-800 border border-slate-700 text-white px-5 py-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-600">
                </div>

                <!-- Developer -->
                <div>
                    <label for="developer" class="block text-slate-400 text-xs font-black uppercase tracking-widest mb-3">Vývojář *</label>
                    <input type="text" id="developer" name="developer" required placeholder="Epic Games, Bungie..."
                           class="w-full bg-slate-800 border border-slate-700 text-white px-5 py-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-600">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="publisher" name="publisher" required placeholder="Microsoft, EA..."
                           class="w-full bg-slate-800 border border-slate-700 text-white px-5 py-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-600">
                </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 max-h-48 overflow-y-auto custom-scrollbar">
                            <?php foreach ($tags as $t): ?>
                                <label class="flex items-center space-x-3 cursor-pointer group p-2 rounded-lg hover:bg-slate-800/50 transition-colors">
                                    <input type="checkbox" name="tags[]" value="<?= $t['id'] ?>" class="tag-checkbox w-4 h-4 text-indigo-500 border-slate-600 bg-slate-900 rounded focus:ring-indigo-500 focus:ring-offset-slate-900 transition-all">
                ...
                <div>
                    ...
                    <input type="number" id="year" name="year" required value="<?= date('Y') ?>" min="1970" max="2035"
                           class="w-full bg-slate-800 border border-slate-700 text-white px-5 py-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="price" name="price" required step="0.01" value="0" min="0"
                           class="w-full bg-slate-800 border border-slate-700 text-white px-5 py-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                </div>

                <!-- Link -->
                <div class="md:col-span-2">
                    <label for="link" class="block text-slate-400 text-xs font-black uppercase tracking-widest mb-3">Odkaz do obchodu (Steam, atd.)</label>
                    <input type="url" id="link" name="link" placeholder="https://store.steampowered.com/..."
                           class="w-full bg-slate-800 border border-slate-700 text-white px-5 py-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-600">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="5" placeholder="Krátký příběh nebo popis hratelnosti..."
                              class="w-full bg-slate-800 border border-slate-700 text-white px-5 py-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-600"></textarea>
                </div>

                <!-- Images -->
                <div class="md:col-span-2">
                    <label class="block text-slate-400 text-xs font-black uppercase tracking-widest mb-3">Obrázky hry</label>
            ...
            <div class="mt-12 flex flex-col md:flex-row gap-4">
                <button type="submit" class="flex-grow bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase tracking-widest py-5 rounded-2xl transition-all shadow-2xl hover:shadow-[0_10px_40px_rgba(79,70,229,0.4)]">
                    ULOŽIT HRU DO DATABÁZE
                </button>
                <a href="<?= BASE_URL ?>/index.php" class="bg-slate-800 hover:bg-slate-700 text-white font-black uppercase tracking-widest px-8 py-5 rounded-2xl transition-all border border-slate-700">
                    ZRUŠIT
                </a>
            </div>
        </form>
