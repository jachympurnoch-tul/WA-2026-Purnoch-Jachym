...
class AuthController {
    ...
    public function storeUser() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Kontrola síly hesla: min 8 znaků, alespoň 1 číslo a 1 velké písmeno
            if (strlen($password) < 8 || !preg_match('/[0-9]/', $password) || !preg_match('/[A-Z]/', $password)) {
                $this->addErrorMessage('Heslo není dostatečně silné. Musí obsahovat alespoň 8 znaků, 1 číslo a 1 velké písmeno.');
                header('Location: ' . BASE_URL . '/index.php?url=auth/register');
                exit;
            }
            ...
            if ($userModel->findByUsername($username)) {
                $this->addErrorMessage('Uživatel s tímto uživatelským jménem již existuje.');
                header('Location: ' . BASE_URL . '/index.php?url=auth/register');
                exit;
            }
    ...
    public function authenticate() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Najdeme uživatele podle emailu nebo uživatelského jména
            $user = $userModel->findByIdentifier($loginId);
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                // ZMĚNA: Uložíme informaci, zda je uživatel admin
                ...
                // CHYBA: Záměrně neříkáme, zda byl špatný login, nebo heslo (bezpečnost!)
                $this->addErrorMessage('Nesprávné údaje nebo heslo.');
                header('Location: ' . BASE_URL . '/index.php?url=auth/login');
                exit;
            }
        }
